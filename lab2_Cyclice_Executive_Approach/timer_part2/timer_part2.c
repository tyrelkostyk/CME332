/*
CME332 Lab 2 Part 2 - Tuesday, Febuary 12th 2019
...
/////* Define Global / External Constants and Mem-Mapped Pointers */////
...
volatile int RUN_flag = 1;  // 0 = Stop the stopwatch, 1 = Run
volatile int time_MM, time_SS;  // Time, in minutes/seconds, of the stopwatch
volatile int time_QS;  // Time, in "quarter seconds", 0.25s. The accuracy of the timer
...
int main(void) {
  ...
  *HEX7_HEX4_ptr &= ~0xFFFFFFFF;
  ...
  // initialize interval timer w/ interrupts
  int_timer_load(INT_TIMER_PERIOD);  // (1 / 50MHz) x 12,500,000 = 0.25s
  int_timer_run(INT_TIMER_CONFIG);   // Config for Int Timer; STOP = 0, START = 1, CONT = 1, ITO = 1

  // enable interrupts for all pushbuttons
  *(KEY_ptr + 2) = 0x3;

  /* set interrupt for levels 0 (interval timer) and level 1 (pushbuttons) */
